...
module SubgroupToCovering (A : Pointed ℓ-zero) ((subgroup BG Bi Bi⋆ isBi isCon isGrp) : SubGroupπ₁ A) where
  X : Type
  X = pullbackΣ {C = ∥ ⟨ A ⟩ ∥ 3} Bi ∣_∣
  ...
  isSet-fib-Bi a = isBi ∣ a ∣

  p-isCov : (a : ⟨ A ⟩) → isSet (fiber p a)
  p-isCov a = subst⁻ isSet (fib-p≡fib-Bi a) (isSet-fib-Bi a)
  ...
  Ã-paths≡Ã-pullback : (a : ⟨ A ⟩) → UniversalCovering.Ã (⟨ A ⟩ , a) ≡ Pullback (pick ∣ a ∣) ∣_∣
  Ã-paths≡Ã-pullback a =
        Σ ⟨ A ⟩ (λ a' → ∥ a ≡ a' ∥ 2)
      ≡⟨ Σ-cong-snd (λ _ → PathIdTrunc 2 ⁻¹) ⟩
        Σ ⟨ A ⟩ (λ a' → ∣ a ∣ ≡ ∣ a' ∣)
  ...
  isConnected'Ã = ∥-∥ₕ-elim (λ x → isSet→isGroupoid (isProp→isSet isConnected'IsProp)) λ a → isConnected'Ã∣a∣ a where
    ...
    isConnected'Ã∣a∣ a = subst isConnected' (Ã-paths≡Ã-pullback a) (UniversalCovering.connected (⟨ A ⟩ , a))
  ...
  connected = subst isConnected' (sym X≡Σfibu) (isConnected'Σ isCon isConnected'-fibu)

  coveringA : Covering A
  coveringA = covering (X , x) p refl p-isCov connected
