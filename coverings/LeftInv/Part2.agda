...
module LeftInv.Part2 (A : Pointed ℓ-zero) (conA : isConnected' ⟨ A ⟩) (((BG , Bi), Bi-⋆ , conBG , grpBG , Bi-fib) : SubGroupπ₁' A) where
  open import LeftInv.Base A conA ((BG , Bi), Bi-⋆ , conBG , grpBG , Bi-fib)

  congP2 : congP (λ i → tr∘Bi∘tr≡Bi∘s i) (transportIsoToPath ⟨BG⟩≅∥X∥ (pt BG)) ≡
    transportRefl (Bi (transport⁻ ⟨BG⟩≡∥X∥ (transport ⟨BG⟩≡∥X∥ (pt BG))))
    ∙ cong Bi (transport⁻Transport ⟨BG⟩≡∥X∥ (pt BG) ∙ s∘t (pt BG) ⁻¹)
  congP2 = {!!}
